...
namespace Ordering.Domain
{
    ...
    public sealed class Order : AggregateRoot
    {
        ...
        public static Order Place(Guid customerId, List<OrderLine> lines)
        {
            ...
            order.Total = lines.Select(l => l.Total)
                .Aggregate(Money.Zero("ZAR"), (acc, x) => acc.Add(x));

            order.Raise(new Events.OrderPlaced(
                order.Id,
                order.CustomerId,
                order.Total.Amount,
                order.Total.Currency,
                order.Lines.Select(l => new Events.OrderLineDto(l.Sku, l.Quantity, l.UnitPrice.Amount)).ToList()
            ));
        ...
        public void Accept()
        {
            if (Status != OrderStatus.Placed)
                throw new InvalidOperationException("Only placed orders can be accepted.");
    ...
    public sealed class OrderLine
    {
        ...
        public Money UnitPrice { get; private set; } = Money.Zero("ZAR");
